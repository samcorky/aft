...
- For local development/testing, ALWAYS use compose.dev.yml and ALWAYS include --build so code changes are reflected:
  - Start/rebuild: docker compose -f compose.dev.yml up -d --build
  - Stop: docker compose -f compose.dev.yml down
  - Reset DB data dir: docker compose -f compose.dev.yml down; remove ./data contents; docker compose -f compose.dev.yml up -d --build
- Do not use compose.yml for development testing (it pulls GHCR images intended for deployment/runtime).
...
Useful commands
- Rebuild dev stack: docker compose -f compose.dev.yml down; docker compose -f compose.dev.yml up -d --build
- Quick dev command reference:
  - Start/rebuild dev stack: docker compose -f compose.dev.yml up -d --build
  - Stop dev stack: docker compose -f compose.dev.yml down
  - Restart a clean DB: docker compose -f compose.dev.yml down; remove ./data contents; docker compose -f compose.dev.yml up -d --build
  - Show service status: docker compose -f compose.dev.yml ps
  - Follow logs: docker compose -f compose.dev.yml logs -f server nginx db redis
  - Rebuild a single service: docker compose -f compose.dev.yml up -d --build server
