...
def upgrade():
    ...
    if 'settings' in inspector.get_table_names():
        ...
        result = conn.execute(sa.text("SELECT COUNT(*) FROM settings WHERE `key` = :key_val"), {'key_val': 'selected_theme'})
        count = result.fetchone()[0]
        
        if count == 0:
            # Insert the selected_theme setting
            op.execute(f"INSERT INTO settings (`key`, `value`) VALUES ('selected_theme', {default_theme_id})")
...
def downgrade():
    ...
    if 'settings' in inspector.get_table_names():
        op.execute("DELETE FROM settings WHERE `key` = 'selected_theme'")
